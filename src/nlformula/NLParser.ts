  ...
  private patterns: Record<'en' | 'vi', PatternMap> = {
    ...
    en: {
      ...
      countif: [
        ...
        /^how\s+many\s+(?:where|when|if)\s+(.+?)\s+=\s+(.+)$/i,
      ],
      averageif: [
      ...
      // Date
      today: [/^(?:today(?:'s)?\s+date|current\s+date)$/i],
      now: [/^(?:now|current\s+(?:date\s+and\s+)?time)$/i],
      datediff: [
    ...
    vi: {
      ...
      today: [/^(?:ngày\s+)?hôm\s+nay$/i],
      now: [/^(?:thời\s+gian\s+)?hiện\s+tại$/i],
      datediff: [
