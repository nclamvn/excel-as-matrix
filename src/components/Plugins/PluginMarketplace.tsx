...
export const PluginMarketplace: React.FC = () => {
  ...
  return (
    <div className={`h-full flex flex-col ${isDark ? 'text-neutral-200' : 'text-neutral-800'}`} data-testid="plugin-marketplace">
      {/* Header */}
      <div className={`px-4 py-3 border-b ${isDark ? 'border-neutral-700' : 'border-neutral-200'}`}>
        <h2 className="text-sm font-semibold flex items-center gap-2 mb-3">
          ...
          Extensions
          {counts.enabled > 0 && (
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${isDark ? 'bg-emerald-900/30 text-emerald-400' : 'bg-emerald-50 text-emerald-700'}`}>
        ...
        <div className="flex gap-1 mb-3">
          <TabButton label="Browse" active={activeTab === 'browse'} onClick={() => setActiveTab('browse')} isDark={isDark} />
          <TabButton label={`Installed (${installed.length})`} active={activeTab === 'installed'} onClick={() => setActiveTab('installed')} isDark={isDark} />
        </div>

        {/* Search */}
        <div className="relative">
          <Search size={14} className={`absolute left-2.5 top-1/2 -translate-y-1/2 ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`} />
          <input
            type="text"
            value={searchQuery}
        ...
        {activeTab === 'browse' && (
          <div className="flex gap-1 mt-2 flex-wrap">
            <CategoryChip label="All" active={categoryFilter === 'all'} onClick={() => setCategoryFilter('all')} isDark={isDark} />
...
const TabButton: React.FC<{ label: string; active: boolean; onClick: () => void; isDark: boolean }> = ({ label, active, onClick, isDark }) => (
  <button type="button"
    onClick={onClick}
    className={`px-3 py-1 text-xs font-medium rounded-md transition-colors
      ${active
        ? isDark ? 'bg-emerald-600 text-white' : 'bg-emerald-600 text-white'
        : isDark ? 'text-neutral-400 hover:bg-neutral-700' : 'text-neutral-500 hover:bg-neutral-100'
      }`}
  >
    {label}
  </button>
...
const CategoryChip: React.FC<{ label: string; active: boolean; onClick: () => void; isDark: boolean }> = ({ label, active, onClick, isDark }) => (
  <button type="button"
    onClick={onClick}
...
const PluginCard: React.FC<{
  ...
  return (
    <div className={`p-3 rounded-lg border ${isDark ? 'border-neutral-700 bg-neutral-800/50' : 'border-neutral-200 bg-white'}`}>
      <div className="flex items-start gap-3">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${isDark ? 'bg-neutral-700' : 'bg-neutral-100'}`}>
          {cat.icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-xs font-semibold truncate">{manifest.name}</h4>
            <span className={`text-[10px] ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>v{manifest.version}</span>
          </div>
          <p className={`text-[11px] mt-0.5 line-clamp-2 ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}>
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className={`flex items-center gap-0.5 text-[10px] ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>
              <Shield size={10} />
        ...
        <button type="button"
          ...
          className={`flex-shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors
            ${isInstalled
              ? isDark ? 'bg-neutral-700 text-neutral-500 cursor-default' : 'bg-neutral-100 text-neutral-400 cursor-default'
