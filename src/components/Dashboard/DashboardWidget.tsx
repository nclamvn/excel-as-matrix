...
export const DashboardWidget: React.FC<DashboardWidgetProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="h-full overflow-hidden" style={{ padding: widget.style.padding }}>
        {renderContent()}
      </div>
...
// KPI Widget Content
const KPIContent: React.FC<{ widget: WidgetType; theme: DashboardTheme }> = ({ widget, theme }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full">
      ...
      {kpi.comparisonType !== 'None' && (
        <div
          className={`flex items-center gap-1 mt-2 ${isPositive ? 'text-green-600' : 'text-red-600'}`}
        >
...
const ImageContent: React.FC<{ widget: WidgetType }> = ({ widget }) => {
  if (!widget.config.imageUrl) {
    return (
      <div className="flex items-center justify-center h-full text-gray-400">No image URL</div>
    );
  }
