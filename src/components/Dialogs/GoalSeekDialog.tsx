  ...
  const handleSeek = useCallback(() => {
    ...
    try {
      const seekResult = goalSeek({
        ...
        targetValue: target,
        initialGuess:
          Number(sheet.cells[getCellKey(changeCellRef.row, changeCellRef.col)]?.value) || 0,
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-xl w-96">
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          {result && (
            <div
              className={`p-2 rounded text-sm ${result.found ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300'}`}
            >
              {result.found
