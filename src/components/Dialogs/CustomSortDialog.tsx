...
export const CustomSortDialog: React.FC<CustomSortDialogProps> = ({ onClose }) => {
  const [sortLevels, setSortLevels] = useState<SortLevel[]>([{ column: 0, direction: 'asc' }]);
  ...
  const maxCol = selectionRange ? Math.max(selectionRange.end.col, 25) : 25;
  ...
  return (
    <div className="dialog-overlay" onClick={onClose}>
      <div
        className="dialog custom-sort-dialog"
          ...
          <div className="sort-levels">
            {sortLevels.map((level, index) => (
              <div key={index} className="sort-level">
                <span className="sort-level-label">{index === 0 ? 'Sort by' : 'Then by'}</span>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateLevel(index, 'direction', e.target.value as 'asc' | 'desc')
                  }
                  className="sort-level-select"
                >
                ...
                <button
                  ...
                  onClick={() =>
                    updateLevel(index, 'direction', level.direction === 'asc' ? 'desc' : 'asc')
                  }
                >
