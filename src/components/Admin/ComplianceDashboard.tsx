...
export const ComplianceDashboard: React.FC = () => {
  ...
  return (
    <div
      className={`p-6 max-w-4xl mx-auto ${isDark ? 'text-neutral-200' : 'text-neutral-800'}`}
    ...
    >
      ...
      {error && (
        <div
          className={`p-3 rounded-lg mb-4 text-sm ${isDark ? 'bg-rose-900/20 text-rose-400' : 'bg-rose-50 text-rose-600'}`}
        >
      ...
      {data && (
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-4xl font-bold font-mono">{data.summary.score}%</div>
          ...
          <div
            className={`mt-3 text-xs font-medium ${data.summary.ready ? 'text-emerald-600' : 'text-amber-600'}`}
          >
            {data.summary.ready
              ? 'Audit-ready: all critical controls passing'
              : 'Action required: resolve failing controls before audit'}
          </div>
        </div>
      )}
      ...
      {data &&
        Object.entries(grouped).map(([category, controls]) => {
          ...
          return (
            <div
              key={category}
              className={`mb-3 rounded-lg border ${isDark ? 'border-neutral-700' : 'border-neutral-200'}`}
            >
              <button
                type="button"
              ...
              {isExpanded && (
                <div
                  className={`px-4 pb-3 border-t ${isDark ? 'border-neutral-700' : 'border-neutral-200'}`}
                >
                  {controls.map((ctrl) => {
                      ...
                      <div key={ctrl.id} className="flex items-start gap-3 py-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-xs font-mono ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}
                            >
                              {ctrl.id}
                            </span>
                          ...
                          <p
                            className={`text-xs mt-1 font-mono ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}
                          >
                            Evidence: {ctrl.evidence}
                          </p>
                        </div>
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${st.color} ${st.bg}`}
                        >
                          {st.label}
                        </span>
                      </div>
