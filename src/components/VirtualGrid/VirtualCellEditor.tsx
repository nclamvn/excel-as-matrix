...
export const VirtualCellEditor = memo<VirtualCellEditorProps>(
  ({ row, col, initialValue, position, onSubmit, onCancel }) => {
    ...
    const handleAutocompleteSelect = useCallback(
      (_suggestion: AutocompleteSuggestion, insertText: string) => {
...
export const FORMULA_SUGGESTIONS: FormulaSuggestion[] = [
  ...
  {
    name: 'IF',
    description: 'Performs conditional logic',
  ...
  {
    name: 'VLOOKUP',
    description: 'Looks up value in first column',
    syntax: '=VLOOKUP(value, range, col, exact)',
    category: 'Lookup',
  },
  {
    name: 'HLOOKUP',
    description: 'Looks up value in first row',
    syntax: '=HLOOKUP(value, range, row, exact)',
    category: 'Lookup',
  },
  {
    name: 'INDEX',
    description: 'Returns value at row/col intersection',
...
export function filterFormulaSuggestions(input: string): FormulaSuggestion[] {
  ...
  return FORMULA_SUGGESTIONS.filter(
    (s) => s.name.startsWith(query) || s.description.toLowerCase().includes(query.toLowerCase())
  ).slice(0, 10);
}
